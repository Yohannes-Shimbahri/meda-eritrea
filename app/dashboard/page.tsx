...
export default function ClientDashboard() {
  ...
  return (
    <main style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: '#f5f0e8' }}>
      ...
      {/* NAVBAR */}
      <nav style={{ borderBottom: '1px solid #222', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, backgroundColor: 'rgba(10,10,10,0.92)', backdropFilter: 'blur(12px)' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 2rem' }}>
        ...
        <div style={{ marginBottom: '2.5rem', animation: 'fadeInUp 0.5s ease both' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '0.4rem' }}>Welcome back, {name.split(' ')[0]} 👋</h1>
          <p style={{ color: '#888', fontSize: '0.95rem' }}>Manage your bookings and saved businesses</p>
        </div>

        {/* STATS */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2.5rem', animation: 'fadeInUp 0.5s ease 0.05s both' }}>
          {[
        ...
        {/* QUICK ACTIONS */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2.5rem', animation: 'fadeInUp 0.5s ease 0.1s both' }}>
          {[
            ...
            item.href ? (
              ...
              <div key={i} onClick={item.action} style={{ backgroundColor: '#111', border: `1px solid ${activeTab === (i === 1 ? 'bookings' : 'saved') ? '#c9933a' : '#222'}`, borderRadius: '1rem', padding: '1.25rem', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '0.5rem', transition: 'border-color 0.2s' }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = '#c9933a')} onMouseLeave={e => (e.currentTarget.style.borderColor = activeTab === (i === 1 ? 'bookings' : 'saved') ? '#c9933a' : '#222')}>
                <span style={{ fontSize: '1.75rem' }}>{item.icon}</span>
                <span style={{ fontWeight: '600', fontSize: '0.9rem' }}>{item.label}</span>
              </div>
            )
        ...
        <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1.25rem', overflow: 'hidden', animation: 'fadeInUp 0.5s ease 0.15s both' }}>
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #222', display: 'flex', gap: '0.5rem' }}>
            {([{ id: 'bookings', label: `📅 My Bookings`, count: bookings.length }, { id: 'saved', label: '❤️ Saved Businesses', count: savedBusinesses.length }] as const).map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '0.6rem 1.25rem', borderRadius: '0.75rem', fontWeight: '600', fontSize: '0.9rem', cursor: 'pointer', border: 'none', backgroundColor: activeTab === tab.id ? '#c9933a' : 'transparent', color: activeTab === tab.id ? '#0a0a0a' : '#888', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                {tab.label}
                    ...
                    <div key={booking.id} style={{ backgroundColor: '#0a0a0a', border: '1px solid #222', borderRadius: '1rem', padding: '1.25rem' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ color: '#888', fontSize: '0.85rem' }}>
                          📅 {new Date(booking.date).toLocaleDateString('en-CA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                          {booking.startTime && <span style={{ color: '#c9933a', marginLeft: '0.5rem' }}>🕐 {booking.startTime}</span>}
                        </div>
                        <Link href={`/business/${booking.business?.slug}`} style={{ color: '#c9933a', fontSize: '0.8rem', textDecoration: 'none', fontWeight: '600' }}>View Business →</Link>
                      </div>
                    ...
                    <div key={biz.id} style={{ backgroundColor: '#0a0a0a', border: '1px solid #222', borderRadius: '1rem', overflow: 'hidden', transition: 'border-color 0.2s' }}
                      ...
                      <div style={{ padding: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          ...
                          <button onClick={() => handleUnsave(biz.id)} disabled={unsaving === biz.id} style={{ padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #333', backgroundColor: 'transparent', color: '#888', cursor: 'pointer', fontSize: '0.8rem' }}>
                            {unsaving === biz.id ? '...' : '♥ Remove'}
                          </button>
                        </div>
                      </div>
                    </div>
