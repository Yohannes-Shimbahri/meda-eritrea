...
function jitterCoords(lat: number, lng: number, index: number) {
  ...
  return {
    lat: lat + r * Math.cos(angle * Math.PI / 180),
    lng: lng + r * Math.sin(angle * Math.PI / 180),
  }
}
...
function GoogleMap({ businesses, selectedId, onSelect, city }: {
  businesses: Business[]
    ...
    businesses.forEach((biz, i) => {
      ...
      const marker = new window.google.maps.Marker({
        ...
        icon: {
          ...
          scale: isSelected ? 14 : isPro ? 11 : 9,
          fillColor: isSelected ? '#ffffff' : isPro ? '#c9933a' : '#555555',
          fillOpacity: 1,
          strokeColor: isSelected ? '#c9933a' : '#0a0a0a',
      ...
      marker.addListener('click', () => {
        ...
        infoWindowRef.current.setContent(`
          <div style="background:#111;color:#f5f0e8;padding:12px 16px;border-radius:10px;min-width:180px;font-family:sans-serif;border:1px solid #333;">
            <div style="font-weight:700;font-size:0.95rem;margin-bottom:4px;">${biz.name}</div>
            <div style="color:#888;font-size:0.8rem;margin-bottom:4px;">${biz.category.replace(/_/g,' ')} · ${biz.city}</div>
            ${biz.rating ? `<div style="color:#f5c842;font-size:0.8rem;">★ ${biz.rating} (${biz.reviewCount})</div>` : '<div style="color:#555;font-size:0.8rem;">New listing</div>'}
            <a href="/business/${biz.slug}" style="display:block;margin-top:8px;background:#c9933a;color:#0a0a0a;padding:5px 10px;border-radius:6px;text-align:center;font-weight:700;font-size:0.8rem;text-decoration:none;">View Profile →</a>
          </div>
        `)
  ...
  useEffect(() => {
    ...
    if (!document.getElementById('gmaps-script')) {
      ...
      const check = setInterval(() => {
        if (window.google?.maps) { clearInterval(check); buildMap() }
      }, 100)
    }
  }, [buildMap])
  ...
  const goToMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      new window.google.maps.Marker({
        position: loc, map: mapInstanceRef.current, title: 'You',
        icon: { path: window.google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: '#60a5fa', fillOpacity: 1, strokeColor: '#fff', strokeWeight: 3 },
        zIndex: 9999,
      })
    })
  }
  ...
  if (!process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY) {
    return (
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        <p style={{ color: '#888', fontSize: '0.85rem', textAlign: 'center', padding: '0 2rem' }}>
          Add <code style={{ color: '#c9933a' }}>NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</code> to .env.local to enable map
        </p>
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <button onClick={goToMyLocation} style={{
        position: 'absolute', bottom: '5rem', right: '0.75rem', zIndex: 10,
        backgroundColor: '#0a0a0a', border: '1px solid #444', borderRadius: '0.75rem',
        padding: '0.6rem 1rem', color: '#f5f0e8', cursor: 'pointer',
        fontSize: '0.82rem', fontWeight: '600', boxShadow: '0 4px 12px rgba(0,0,0,0.6)',
      ...
      <div style={{
        ...
        backgroundColor: 'rgba(10,10,10,0.88)', border: '1px solid #333',
        borderRadius: '0.75rem', padding: '0.4rem 0.75rem',
        fontSize: '0.75rem', color: '#888', backdropFilter: 'blur(8px)',
      }}>
      ...
      <div style={{
        ...
        <span>⬤ <span style={{ color: '#c9933a' }}>PRO</span></span>
        <span>⬤ <span style={{ color: '#555' }}>Standard/Free</span></span>
        <span>⬤ <span style={{ color: '#fff' }}>Selected</span></span>
      </div>
    </div>
  )
}
...
export default function BrowsePage() {
  ...
  const filtered = businesses
    .filter(b => {
      ...
      return (
        ...
        (!category || b.category.toLowerCase().replace(/\s+/g, '-') === category) &&
  ...
  return (
    <main style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: '#f5f0e8', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* NAVBAR */}
      <nav style={{ borderBottom: '1px solid #222', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 200, backgroundColor: 'rgba(10,10,10,0.95)', backdropFilter: 'blur(12px)' }}>
        <Link href="/" style={{ fontSize: '1.5rem', fontWeight: '800', color: '#c9933a', textDecoration: 'none' }}>Meda</Link>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          {user ? (
            <>
              <Link href={dashboardHref} style={{ color: '#888', fontSize: '0.9rem', textDecoration: 'none' }}>My Dashboard</Link>
              <button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/' }} style={{ background: 'none', border: '1px solid #333', color: '#888', padding: '0.5rem 1rem', borderRadius: '0.75rem', cursor: 'pointer', fontSize: '0.9rem' }}>Sign Out</button>
            ...
            <>
              ...
              <Link href="/register/client" style={{ backgroundColor: '#c9933a', color: '#0a0a0a', padding: '0.5rem 1.25rem', borderRadius: '0.75rem', fontWeight: '700', fontSize: '0.9rem', textDecoration: 'none' }}>Sign Up</Link>
            </>
          )}
        </div>
      </nav>
      ...
      <div style={{ backgroundColor: '#111', borderBottom: '1px solid #222', padding: '1.5rem 2rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: '800', marginBottom: '1rem' }}>Browse Habesha Businesses</h1>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search businesses..."
            style={{ flex: 1, minWidth: '200px', background: '#0a0a0a', border: '1px solid #333', borderRadius: '0.75rem', padding: '0.75rem 1rem', color: '#f5f0e8', fontSize: '0.9rem', outline: 'none' }}
            onFocus={e => (e.currentTarget.style.borderColor = '#c9933a')}
            onBlur={e => (e.currentTarget.style.borderColor = '#333')} />
          <select value={category} onChange={e => setCategory(e.target.value)} style={{ background: '#0a0a0a', border: '1px solid #333', borderRadius: '0.75rem', padding: '0.75rem 1rem', color: '#f5f0e8', fontSize: '0.9rem', outline: 'none' }}>
            {categories.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
          </select>
          <select value={city} onChange={e => setCity(e.target.value)} style={{ background: '#0a0a0a', border: '1px solid #333', borderRadius: '0.75rem', padding: '0.75rem 1rem', color: '#f5f0e8', fontSize: '0.9rem', outline: 'none' }}>
            {cities.map(c => <option key={c}>{c}</option>)}
          </select>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ background: '#0a0a0a', border: '1px solid #333', borderRadius: '0.75rem', padding: '0.75rem 1rem', color: '#f5f0e8', fontSize: '0.9rem', outline: 'none' }}>
          ...
          <div style={{ display: 'flex', backgroundColor: '#0a0a0a', border: '1px solid #333', borderRadius: '0.75rem', overflow: 'hidden' }}>
            <button onClick={() => setView('grid')} style={{ padding: '0.75rem 1rem', border: 'none', cursor: 'pointer', backgroundColor: view === 'grid' ? '#c9933a' : 'transparent', color: view === 'grid' ? '#0a0a0a' : '#888', fontWeight: '600', fontSize: '0.9rem', transition: 'all 0.2s' }}>⊞ Grid</button>
            <button onClick={() => setView('map')} style={{ padding: '0.75rem 1rem', border: 'none', cursor: 'pointer', backgroundColor: view === 'map' ? '#c9933a' : 'transparent', color: view === 'map' ? '#0a0a0a' : '#888', fontWeight: '600', fontSize: '0.9rem', transition: 'all 0.2s' }}>🗺 Map</button>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', overflowX: 'auto', paddingBottom: '0.25rem' }}>
          {categories.map(c => (
            <button key={c.value} onClick={() => setCategory(c.value)} style={{ padding: '0.4rem 1rem', borderRadius: '2rem', border: '1px solid #333', backgroundColor: category === c.value ? '#c9933a' : 'transparent', color: category === c.value ? '#0a0a0a' : '#888', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600', whiteSpace: 'nowrap', transition: 'all 0.2s', flexShrink: 0 }}>
              {c.label}
            </button>
          ))}
        </div>
      ...
      {/* RESULTS COUNT */}
      <div style={{ padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: view === 'map' ? '1px solid #222' : 'none' }}>
        <p style={{ color: '#888', fontSize: '0.9rem', margin: 0 }}>
          {loadingData ? 'Searching...' : (
            <><span style={{ color: '#f5f0e8', fontWeight: '700' }}>{filtered.length}</span> businesses found
              {city !== 'All Cities' && <> in <span style={{ color: '#c9933a' }}>{city}</span></>}
              {category && <> · <span style={{ color: '#c9933a' }}>{categories.find(c => c.value === category)?.label}</span></>}
              {!usingRealData && <span style={{ color: '#555', fontSize: '0.8rem' }}> · sample listings</span>}
            </>
          )}
        </p>
        {category && (
          <button onClick={() => setCategory('')} style={{ background: 'none', border: '1px solid #333', color: '#888', padding: '0.35rem 0.75rem', borderRadius: '1rem', cursor: 'pointer', fontSize: '0.8rem' }}>✕ Clear filter</button>
        )}
      </div>

      {/* ── GRID VIEW ── */}
      {view === 'grid' && (
        <div style={{ padding: '1.5rem 2rem 3rem', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {loadingData ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1.25rem', overflow: 'hidden', animation: `pulse 1.5s ease ${i * 0.1}s infinite` }}>
              ...
              <Link key={business.id} href={`/business/${business.slug}`} style={{ textDecoration: 'none', color: '#f5f0e8', animation: `fadeInUp 0.4s ease ${i * 0.05}s both` }}>
                <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1.25rem', overflow: 'hidden', transition: 'all 0.3s', cursor: 'pointer', height: '100%' }}
                  ...
                  <div style={{ height: '180px', position: 'relative', overflow: 'hidden', backgroundColor: '#222' }}>
                    {business.coverImage ? <Image src={business.coverImage} alt={business.name} fill style={{ objectFit: 'cover' }} /> : (
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3.5rem' }}>{getCategoryEmoji(business.category)}</div>
                    )}
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.6) 0%, transparent 60%)' }} />
                    <div style={{ position: 'absolute', top: '0.75rem', left: '0.75rem', display: 'flex', gap: '0.4rem' }}>
                      {business.subscription === 'PRO' && <span style={{ backgroundColor: '#c9933a', color: '#0a0a0a', padding: '0.2rem 0.6rem', borderRadius: '1rem', fontSize: '0.7rem', fontWeight: '800' }}>⭐ PRO</span>}
                      {business.isVerified && <span style={{ backgroundColor: 'rgba(0,0,0,0.7)', color: '#4ade80', padding: '0.2rem 0.6rem', borderRadius: '1rem', fontSize: '0.7rem', fontWeight: '700' }}>✓ Verified</span>}
                    </div>
                  </div>
                  <div style={{ padding: '1.25rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                      <h3 style={{ fontWeight: '700', fontSize: '1rem', margin: 0 }}>{business.name}</h3>
                      {business.rating ? (
                      ...
                      ) : <span style={{ color: '#555', fontSize: '0.8rem', flexShrink: 0, marginLeft: '0.5rem' }}>New</span>}
                    </div>
                    ...
                    <div style={{ padding: '0.6rem 1rem', backgroundColor: '#c9933a', borderRadius: '0.75rem', textAlign: 'center', fontWeight: '700', fontSize: '0.85rem', color: '#0a0a0a' }}>View Profile</div>
                  </div>
                </div>
              </Link>
            ))}
      ...
      {view === 'map' && (
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', height: 'calc(100vh - 260px)', minHeight: '550px' }}>

          {/* LEFT PANEL — scrollable card list */}
          <div style={{ width: '360px', flexShrink: 0, overflowY: 'auto', borderRight: '1px solid #222', backgroundColor: '#080808' }}>
            <div style={{ padding: '0.75rem' }}>
              {filtered.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#888' }}>
                  <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>🔍</div>
                  <p style={{ fontSize: '0.9rem' }}>No businesses found</p>
                </div>
              ) : filtered.map(biz => (
                <div
                  key={biz.id}
                  ...
                  style={{
                    backgroundColor: selectedId === biz.id ? '#1a1200' : '#111',
                    border: `1px solid ${selectedId === biz.id ? '#c9933a' : '#222'}`,
                    borderRadius: '0.875rem', padding: '0.875rem',
                    marginBottom: '0.5rem', cursor: 'pointer', transition: 'all 0.15s',
                    boxShadow: selectedId === biz.id ? '0 0 0 1px rgba(201,147,58,0.25)' : 'none',
                  }}
                  onMouseEnter={e => { if (selectedId !== biz.id) e.currentTarget.style.borderColor = '#444' }}
                  onMouseLeave={e => { if (selectedId !== biz.id) e.currentTarget.style.borderColor = '#222' }}
                >
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                    <div style={{ width: '50px', height: '50px', borderRadius: '0.625rem', overflow: 'hidden', flexShrink: 0, backgroundColor: '#222', position: 'relative' }}>
                      {biz.coverImage ? <Image src={biz.coverImage} alt={biz.name} fill style={{ objectFit: 'cover' }} /> : (
                        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>{getCategoryEmoji(biz.category)}</div>
                      )}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.2rem', flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: '700', fontSize: '0.88rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{biz.name}</span>
                        {biz.subscription === 'PRO' && <span style={{ backgroundColor: '#c9933a', color: '#0a0a0a', padding: '0.1rem 0.35rem', borderRadius: '1rem', fontSize: '0.62rem', fontWeight: '800', flexShrink: 0 }}>PRO</span>}
                        {biz.isVerified && <span style={{ color: '#4ade80', fontSize: '0.7rem', flexShrink: 0 }}>✓</span>}
                      </div>
                      <div style={{ color: '#666', fontSize: '0.78rem', marginBottom: '0.4rem' }}>
                        {biz.category.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} · 📍 {biz.city}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        {biz.rating ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                            <span style={{ color: '#f5c842', fontSize: '0.78rem' }}>★</span>
                            <span style={{ fontWeight: '700', fontSize: '0.78rem' }}>{biz.rating}</span>
                            <span style={{ color: '#666', fontSize: '0.72rem' }}>({biz.reviewCount})</span>
                          </div>
                        ) : <span style={{ color: '#444', fontSize: '0.72rem' }}>New listing</span>}
                        <Link href={`/business/${biz.slug}`} onClick={e => e.stopPropagation()}
                          style={{ backgroundColor: '#c9933a', color: '#0a0a0a', padding: '0.28rem 0.65rem', borderRadius: '0.45rem', fontSize: '0.72rem', fontWeight: '700', textDecoration: 'none' }}>
                          View →
                        </Link>
                      </div>
                    </div>
