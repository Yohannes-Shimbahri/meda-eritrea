...
type AnalyticsData = {
  summary: { totalBookings: number; periodBookings: number; totalRevenue: number; periodRevenue: number; completionRate: number; cancelled: number; totalViews: number; periodViews: number; conversionRate: number }
...
function MiniBarChart({ data, valueKey, color = '#c9933a', height = 80 }: { data: { label: string; [key: string]: any }[]; valueKey: string; color?: string; height?: number }) {
    ...
    <div style={{ width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '2px', height: `${height}px` }}>
        {data.map((d, i) => {
          ...
          return <div key={d.date ?? i} title={`${d.label}: ${d[valueKey]}`} style={{ flex: 1, height: `${Math.max(pct, d[valueKey] > 0 ? 4 : 1)}%`, backgroundColor: d[valueKey] > 0 ? color : '#222', borderRadius: '3px 3px 0 0', cursor: 'default', minWidth: 0 }} onMouseEnter={e => (e.currentTarget.style.opacity = '0.75')} onMouseLeave={e => (e.currentTarget.style.opacity = '1')} />
        })}
      </div>
      <div style={{ display: 'flex', gap: '2px', marginTop: '4px' }}>
        {data.map((d, i) => <div key={d.date ?? i} style={{ flex: 1, minWidth: 0, textAlign: 'center', fontSize: '0.6rem', color: '#555', overflow: 'hidden', whiteSpace: 'nowrap' }}>{i % showEvery === 0 ? d.label : ''}</div>)}
      </div>
    </div>
  )
}

function HBar({ label, value, max, color = '#c9933a', suffix = '' }: { label: string; value: number; max: number; color?: string; suffix?: string }) {
...
function AnalyticsStatCard({ icon, label, value, sub, color = '#c9933a', locked = false }: { icon: string; label: string; value: string; sub?: string; color?: string; locked?: boolean }) {
  return (
    <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.25rem', position: 'relative', overflow: 'hidden' }}>
      {locked && (
        <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(10,10,10,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '1rem', zIndex: 1 }}>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: '1.25rem', marginBottom: '0.25rem' }}>🔒</div><div style={{ color: '#c9933a', fontSize: '0.75rem', fontWeight: '700' }}>PRO only</div></div>
        </div>
      )}
...
function AnalyticsTab({ getAuthHeaders, plan, onUpgrade, userId }: { getAuthHeaders: () => Promise<Record<string, string>>; plan: string; onUpgrade: () => void; userId: string | null }) {
  ...
  if (!hasAnalytics) return (
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      ...
      <div style={{ backgroundColor: '#111', border: '1px solid #c9933a44', borderRadius: '1.25rem', padding: '3rem', textAlign: 'center' }}>
        ...
        <p style={{ color: '#888', marginBottom: '1.5rem', fontSize: '0.9rem' }}>Upgrade to Standard for booking trends, top services & revenue estimates.<br />Upgrade to Pro for profile views and conversion rates.</p>
        <button onClick={onUpgrade} style={{ backgroundColor: '#c9933a', color: '#0a0a0a', border: 'none', padding: '0.875rem 2rem', borderRadius: '0.875rem', fontWeight: '700', cursor: 'pointer', fontSize: '1rem' }}>Upgrade to Standard — $19/mo</button>
      </div>
    </div>
  )
  ...
  if (loading) return (
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      ...
      <div className="stats-grid-4" style={{ display: 'grid', gap: '1rem', marginBottom: '1.5rem' }}>
        {[1,2,3,4].map(i => <div key={i} style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', height: '100px' }} />)}
      </div>
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      <div className="analytics-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '0.4rem' }}>Analytics</h1>
          <p style={{ color: '#888', fontSize: '0.9rem' }}>Showing data for the <span style={{ color: '#c9933a' }}>{rangeLabel}</span>
            <span style={{ marginLeft: '0.5rem', color: isPro ? '#c9933a' : '#60a5fa', fontSize: '0.8rem', fontWeight: '700', backgroundColor: isPro ? '#1a1200' : '#0a0a1f', padding: '0.15rem 0.5rem', borderRadius: '1rem', border: `1px solid ${isPro ? '#c9933a44' : '#60a5fa44'}` }}>{plan}</span>
          </p>
        </div>
        <div style={{ display: 'flex', backgroundColor: '#111', border: '1px solid #333', borderRadius: '0.75rem', overflow: 'hidden' }}>
          {[['7d', '7d'], ['30d', '30d'], ['90d', '90d']].map(([val, label]) => (
            <button key={val} onClick={() => setRange(val)} style={{ padding: '0.5rem 0.875rem', border: 'none', cursor: 'pointer', backgroundColor: range === val ? '#c9933a' : 'transparent', color: range === val ? '#0a0a0a' : '#888', fontWeight: '600', fontSize: '0.85rem' }}>{label}</button>
          ))}
        </div>
      </div>

      {!isPro && (
        <div style={{ backgroundColor: '#1a1200', border: '1px solid #c9933a44', borderRadius: '1rem', padding: '1.25rem', marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div><div style={{ fontWeight: '700', color: '#c9933a', marginBottom: '0.25rem' }}>⭐ Unlock Advanced Analytics</div><div style={{ color: '#888', fontSize: '0.85rem' }}>Profile views, conversion rate & revenue breakdown require Pro.</div></div>
          <button onClick={onUpgrade} style={{ backgroundColor: '#c9933a', color: '#0a0a0a', border: 'none', padding: '0.6rem 1.25rem', borderRadius: '0.75rem', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>Upgrade to Pro</button>
        </div>
      )}
      ...
      {isPro && (
        <div className="analytics-pro-row" style={{ display: 'grid', gap: '1rem', marginBottom: '1.5rem' }}>
      ...
      <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div style={{ fontWeight: '700', fontSize: '0.95rem' }}>{chartMode === 'bookings' ? '📅 Booking Trend' : '💰 Revenue Trend'}</div>
      ...
      <div className="two-col-grid" style={{ display: 'grid', gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.5rem' }}>
          ...
          {topServices.length === 0 ? <div style={{ color: '#555', fontSize: '0.85rem', textAlign: 'center', padding: '1.5rem 0' }}>No bookings yet</div> : topServices.map(s => <HBar key={s.name} label={s.name} value={s.count} max={maxBookings} color="#c9933a" />)}
        </div>
        <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.5rem', position: 'relative', overflow: 'hidden' }}>
          ...
          {revenueByService.length === 0 ? <div style={{ color: '#555', fontSize: '0.85rem', textAlign: 'center', padding: '1.5rem 0' }}>No revenue data yet</div> : revenueByService.map(s => <HBar key={s.name} label={s.name} value={s.revenue} max={maxRevenue} color="#4ade80" suffix="$" />)}
        </div>
      </div>
      ...
      {isPro && (
        <div style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.5rem' }}>
          ...
          {viewTrend.every(v => v.views === 0) ? <div style={{ textAlign: 'center', padding: '2rem', color: '#555', fontSize: '0.9rem' }}>No profile views recorded yet.</div> : <MiniBarChart data={viewTrend} valueKey="views" color="#f472b6" height={120} />}
        </div>
      )}
    </div>
  )
...
function PortfolioTab({ getAuthHeaders, plan }: { getAuthHeaders: () => Promise<Record<string, string>>; plan: string }) {
    ...
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: '#888', fontSize: '0.9rem' }}>Showcase your best work<br /><span style={{ color: limit === Infinity ? '#4ade80' : atLimit ? '#e05c5c' : '#c9933a' }}>{limit === Infinity ? `Unlimited photos (${plan} plan)` : `${items.length}/${limit} photos used (${plan} plan)`}</span></p>
        </div>
        <label style={{ backgroundColor: atLimit ? '#333' : '#c9933a', color: atLimit ? '#666' : '#0a0a0a', border: 'none', padding: '0.75rem 1.5rem', borderRadius: '0.75rem', fontWeight: '700', cursor: atLimit ? 'not-allowed' : 'pointer', fontSize: '0.9rem', whiteSpace: 'nowrap' }}>
      ...
      {items.length > 0 && (
        <div className="portfolio-grid" style={{ display: 'grid', gap: '0.875rem' }}>
          {items.map(item => (
      ...
      {items.length === 0 && !uploading && <div style={{ textAlign: 'center', padding: '2rem', color: '#555', fontSize: '0.9rem' }}>No portfolio photos yet</div>}
    </div>
  )
}

function GalleryTab({ getAuthHeaders, plan, saveButtonStyle }: { getAuthHeaders: () => Promise<Record<string, string>>; plan: string; saveButtonStyle: (color?: string) => React.CSSProperties }) {
  ...
  const handleUpload = async (file: File) => {
    if (atLimit) { setMsg(`${plan} plan limit reached`); return }
    setUploading(true)
    const reader = new FileReader()
    reader.onload = async (ev) => {
      const base64 = ev.target?.result as string
    ...
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ color: '#888', fontSize: '0.9rem' }}>Workspace photos. <span style={{ color: '#c9933a' }}>For work samples, use &quot;Our Work&quot;.</span></p>
        </div>
        <label style={{ ...saveButtonStyle(atLimit ? '#333' : '#c9933a'), cursor: atLimit ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}>
      ...
      {media.length > 0 && (
        <div className="portfolio-grid" style={{ display: 'grid', gap: '1rem' }}>
          {media.map(m => (
...
function BookingsTab({ getAuthHeaders }: { getAuthHeaders: () => Promise<Record<string, string>> }) {
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
      ...
      {loading ? <div style={{ textAlign: 'center', padding: '3rem', color: '#888' }}>Loading bookings...</div> : filtered.length === 0 ? (
    ...
    <div style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: '#f5f0e8', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ borderBottom: '1px solid #222', padding: '0.875rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 200, backgroundColor: 'rgba(10,10,10,0.95)', backdropFilter: 'blur(12px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <span className="hide-mobile" style={{ color: '#333' }}>|</span>
          <span className="hide-mobile" style={{ color: '#888', fontSize: '0.85rem' }}>Business Dashboard</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {saveMsg && <span style={{ color: saveMsg.startsWith('✓') ? '#4ade80' : '#e05c5c', fontSize: '0.85rem', fontWeight: '600' }}>{saveMsg}</span>}
          {bookingStats.pending > 0 && (
            <button onClick={() => setActiveTab('bookings')} style={{ background: '#1a1200', border: '1px solid #c9933a', color: '#c9933a', padding: '0.5rem 0.75rem', borderRadius: '0.75rem', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '700' }}>🔔 {bookingStats.pending}</button>
          )}
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        {/* MAIN */}
        <main style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', maxWidth: '1000px', paddingBottom: '5rem' }} className="dash-main">
          ...
          {activeTab === 'overview' && (
            <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
              ...
              <div className="quick-actions-grid" style={{ display: 'grid', gap: '1rem', marginBottom: '2rem' }}>
                {[
          ...
          {activeTab === 'employees' && (
            <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <button onClick={() => {
                  if (atEmpLimit) { showSaveMsg(`${currentPlan} plan limit: ${empLimit === Infinity ? '∞' : empLimit} employees max.`); return }
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {dashEmployees.map((emp, i) => (
                    <div key={emp.id} style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1.25rem' }}>
                      ...
                      <div className="employee-card-grid" style={{ display: 'grid', gap: '1.25rem', alignItems: 'start' }}>
                        <div>
          ...
          {activeTab === 'hours' && (
            <div style={{ animation: 'fadeInUp 0.4s ease both' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {dashHours.map((h, i) => (
                  <div key={h.day} className="hours-row" style={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '1rem', padding: '1rem 1.25rem', display: 'grid', alignItems: 'center', gap: '0.75rem', opacity: h.closed ? 0.5 : 1 }}>
                ...
                {[
                  ...
                  <div key={plan.name} style={{ backgroundColor: '#111', border: `2px solid ${currentPlan === plan.name ? plan.color : '#222'}`, borderRadius: '1rem', padding: '1.5rem', position: 'relative' as const }}>
                    ...
                    {currentPlan === plan.name ? (
                      plan.name !== 'FREE' && <button onClick={handleManageBilling} style={{ width: '100%', backgroundColor: '#222', color: '#f5f0e8', border: 'none', padding: '0.75rem', borderRadius: '0.75rem', fontWeight: '700', cursor: 'pointer' }}>Manage Billing</button>
                    ) : (
                      plan.name !== 'FREE' && <button onClick={() => handleUpgrade(plan.name)} style={{ width: '100%', backgroundColor: plan.color === '#c9933a' ? '#c9933a' : '#222', color: plan.color === '#c9933a' ? '#0a0a0a' : '#f5f0e8', border: 'none', padding: '0.75rem', borderRadius: '0.75rem', fontWeight: '700', cursor: 'pointer' }}>Upgrade to {plan.label}</button>
                    )}
                  </div>
                ))}
              </div>
      ...
      {/* MOBILE BOTTOM NAV */}
      <nav className="mobile-bottom-nav" style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(10,10,10,0.97)', borderTop: '1px solid #222', backdropFilter: 'blur(12px)', zIndex: 300 }}>
        {mobileNavItems.map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.2rem', padding: '0.6rem 0.25rem', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === item.id ? '#c9933a' : '#555' }}>
            <span style={{ fontSize: '1.25rem', position: 'relative' }}>
              {item.icon}
              {item.id === 'bookings' && bookingStats.pending > 0 && (
                <span style={{ position: 'absolute', top: '-4px', right: '-6px', backgroundColor: '#c9933a', color: '#0a0a0a', borderRadius: '50%', width: '14px', height: '14px', fontSize: '0.55rem', fontWeight: '800', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{bookingStats.pending}</span>
              )}
            </span>
            <span style={{ fontSize: '0.65rem', fontWeight: activeTab === item.id ? '700' : '500' }}>{item.label}</span>
          </button>
        ))}
      </nav>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .analytics-pro-row { grid-template-columns: 1fr !important; }
          .employee-card-grid { grid-template-columns: 1fr !important; }
          .hours-row { grid-template-columns: 1fr 1fr !important; }
